...
use PDO;
use App\LtiConsumer;
class LTI_Data_Connector_Eloquent extends LTI_Data_Connector {
...
###
  public function Tool_Consumer_load($consumer) {
    ...
    $result = LtiConsumer::where('consumer_key', $consumer->getKey())->get();

    if ($result){
      $consumer = $result;

      if (!is_null($result->enable_from)){
        $consumer->enable_from = strtotime($result->enable_from);
      }

      $consumer->enable_until = NULL;
      if (!is_null($result->enable_until)){
        $consumer->enable_until = strtotime($result->enable_until);
      }

      $consumer->last_access = NULL;
      if (!is_null($result->last_access)){
        $consumer->last_access = strtotime($result->last_access);
      }

      $consumer->created = strtotime($result->created);
      $consumer->updated = strtotime($result->updated);
...
###
  public function Tool_Consumer_save($consumer) {
    ...
    if (is_null($consumer->created)) {
      ...
      $lti_consumer->key = $key;
      $lti_consumer->protected = $protected;
      $lti_consumer->enabled = $enabled;
      $lti_consumer->enable_from = $enable_from;
      $lti_consumer->enable_until = $enable_until;
      $lti_consumer->last_access = $last_access;
      $lti_consumer->created = $created;
      $lti_consumer->updated = $updated;
    } else {
      $lti_consumer = LtiConsumer::find($key);
      $lti_consumer = $consumer;

      $lti_consumer->protected = $protected;
      $lti_consumer->enabled = $enabled;
      $lti_consumer->enable_from = $from;
      $lti_consumer->enable_until = $until;
      $lti_consumer->last_access = $last;
      $lti_consumer->updated = $now;
    }
    $ok = $lti_consumer->save();
    if ($ok) {
      if (is_null($consumer->created)) {
