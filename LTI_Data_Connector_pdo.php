  ...
  public function User_save($user) {
    ...
    if (is_null($user->created)) {
      $sql = 'INSERT INTO ' . $this->dbTableNamePrefix . LTI_Data_Connector::USER_TABLE_NAME . ' (consumer_key, context_id, ' .
             'user_id, firstname, lastname, fullname, email, roles, lti_result_sourcedid, created, updated) ' .
             'VALUES (:key, :id, :user_id, :firstname, :lastname, :fullname, :email, :roles, :lti_result_sourcedid, :now, :now)';
    } else {
      $sql = 'UPDATE ' . $this->dbTableNamePrefix . LTI_Data_Connector::USER_TABLE_NAME . ' ' .
          'SET lti_result_sourcedid = :lti_result_sourcedid, updated = :now, firstname = :firstname, lastname = :lastname, fullname = :fullname, email = :email, roles = :roles ' .
             'WHERE (consumer_key = :key) AND (context_id = :id) AND (user_id = :user_id)';
    }
    ...
    $query->bindValue('fullname', $user->fullname, PDO::PARAM_STR);
    $query->bindValue('email', $user->email, PDO::PARAM_STR);
    $query->bindValue('roles', json_encode($user->roles), PDO::PARAM_STR);
